...
llvm::Value *Unary::createRightValue(TransformContext &context){
    switch (op->id) {
        ...
        case NOT:
        {
            ...
            return context.builder.CreateIntCast(context.builder.CreateICmpEQ(rightValue, context.builder.getInt32(0)), context.builder.getInt32Ty(), false);
        }
        case AND:
...
void Unary::createConditionBranch(TransformContext &context, llvm::BasicBlock *trueBlock,
    ...
    switch (op->id) {
        case NOT:
            value->createConditionBranch(context, falseBlock, trueBlock);
            return;
    }
